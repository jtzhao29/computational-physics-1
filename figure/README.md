...
而这也是直观的。当$x_n$的值能够稳定振荡时,$x_n$的取值其实是方程的不动点，与初值无关。

当$x_n$的值不能够稳定振荡时，说明系统陷入了混沌系统，系统的长时间行为收到初值的影响。初值的细微差异会导致$x_n$的行为发生剧烈的变化。
...
对于稳定点现象，对于方程一，稳定情况即方程本身的不动点（不稳定解已舍去）：
...
而对于方程二，$x_{n}=\cos x-\mu x_n^2$是超越方程，没有解析形式的解。
...
由上图可以看出，得分与得分频率之间存在幂律关系，在双对数图下的斜率是-1.34。
...
$$p_{s_2} = p_3^2(1-p_3)^6$$

$$p_{s_3} = 2p_3^3(1-p_3)^8$$
